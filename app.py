...
def get_model(name='nmtlr'):
    with open('./outputModel/nmtlr_model_200.pkl', 'rb') as f:   
        model = dill.load(f)
    return model
...
NMTLR_model = get_model(st.session_state['model'])
sidebar_code = get_code()
colors=["#FFA64E","#43A64A","#D61928","#71A5D1"]
def plot_survival():
    pd_data = pd.concat(
        [
            pd.DataFrame(
    ...
    if st.session_state['display']:
        fig = px.line(pd_data, x="Time", y="Survival", color='Patients',range_x=[0,80], range_y=[0, 1],
                      color_discrete_sequence=colors)
    else:
        fig = px.line(pd_data.loc[pd_data['Patients'] == pd_data['Patients'].to_list()[-1], :], x="Time", y="Survival",
                      range_x=[0,80],range_y=[0, 1])
...
def predict_survival(df, end=120):
    time_points = np.arange(1, end + 1)
    survival = [] 
    for t in time_points:
        surv = NMTLR_model.model.predict_survival(df, t=np.array([t]))  
        survival.append(surv.flatten())    
...
def predict():
    ...
    survival=predict_survival(np.array(input),80)
    # 保留120个预测的数据
